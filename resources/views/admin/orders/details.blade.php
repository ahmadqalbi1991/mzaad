            ...
            {{-- <form action="" method="get">
                ...
                <table width="100%">
                    <thead>
                        ...
                        <tr>
                            <th>Vendor</th>
                            <th>: {{$order->vendor->vendordata->company_name ?? ''}}</th>
                            <th>@if($list[0]->order_type == 1) Order Type @else Delivery Address @endif</th>
                            <th>: @if(!empty($list[0]->shipping_address)) <?php $shipdata = \App\Models\UserAdress::get_address_details($list[0]->address_id) ??[];//$list[0]->shipping_address;
                             ?>
                             @if(request()->test)
                             @dd($shipdata)
                             @endif
                        ...
                        <tr>
                         ...
                         <tr>
                            ...
                            <th>: {{number_format($list[0]->shipping_charge, 2, '.', '')}}</th>
                             <th>Service charge</th>
                            <th>: {{number_format($list[0]->service_charge, 2, '.', '')}}</th>
                        ...
                        <tr>
                            ...
                            <th>Payment Mode</th>
                            <th>:{{payment_mode($list[0]->payment_mode)}}</th>
                             <th>Admin Earning</th>
            ...
            <?php } ?>
                ...
                <div class="order-page-infomatics">
                                  ...
                                  <div class="row">
                                    ...
                                    <div class="delivery-status-block">
                                        <ul class="list-unstyled">
                                            ...
                                            <br>{{get_date_in_timezone($list[0]->created_at,'d-M-Y h:i A')}}
                                            </li>
                                ...
                                <div class="action-divs d-flex align-items-center">
                                    @if ( GetUserPermissions("orders_edit") )
                                            @if($list[0]->status != config('global.order_status_cancelled') || $list[0]->status != config('global.order_status_rejected'))
                                            
                                            <div class="cancel_btn">
                                                {{-- <button type="button" class="btn btn-danger">Cancel</button> --}}
                                                 @if($list[0]->status != config('global.order_status_cancelled') || $list[0]->status != config('global.order_status_delivered') || $list[0]->status != config('global.order_status_rejected'))
                                                @if($list[0]->status == config('global.order_status_pending'))
                                                        <span class="btn btn-warning" data-role="status-change" href="{{url('admin/order/change_status')}}" detailsid="{{$list[0]->order_id}}" value="{{config('global.order_status_accepted')}}" >Accept</span>
                                                ...
                                                @if($list[0]->status == config('global.order_status_accepted'))
                                                    <span class="btn btn-design" data-role="status-change" href="{{url('admin/order/change_status')}}" detailsid="{{$list[0]->order_id}}" value="{{config('global.order_status_ready_for_delivery')}}" >Ready for Delivery</span>

                                                    <span class="btn btn-danger" data-role="status-change" href="{{url('admin/order/change_status')}}" detailsid="{{$list[0]->order_id}}" value="{{config('global.order_status_cancelled')}}" >cancel</span>
                                                @endif

                                                @if($list[0]->status == config('global.order_status_ready_for_delivery'))
                                                    <span class="btn btn-info" data-role="status-change" href="{{url('admin/order/change_status')}}" detailsid="{{$list[0]->order_id}}" value="{{config('global.order_status_dispatched')}}" >Dispatched</span>

                                                    <span class="btn btn-danger" data-role="status-change" href="{{url('admin/order/change_status')}}" detailsid="{{$list[0]->order_id}}" value="{{config('global.order_status_cancelled')}}" >cancel</span>
                                                @endif

                                                @if($list[0]->status == config('global.order_status_dispatched'))
                                                    <span class="btn btn-success" data-role="status-change" href="{{url('admin/order/change_status')}}" detailsid="{{$list[0]->order_id}}" value="{{config('global.order_status_delivered')}}" >Delivered</span>
                                                    <span class="btn btn-danger" data-role="status-change" href="{{url('admin/order/change_status')}}" detailsid="{{$list[0]->order_id}}" value="{{config('global.order_status_cancelled')}}" >cancel</span>
                                                @endif
                                                 @endif
                                            </div>
