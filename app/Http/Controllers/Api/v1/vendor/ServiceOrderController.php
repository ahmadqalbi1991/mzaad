...
class ServiceOrderController extends Controller
{
    ...
    public function service_orders(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
           ...
           if(!empty($serar))
            {
            $order_list = OrderServiceModel::select('orders_services.order_id','orders_services.order_no','orders_services_items.id as id','orders_services_items.order_status','orders_services_items.total as price',
           ...
           if(is_numeric($request->status) && $request->status >= 0 && $request->status != 11)
            {
              $order_list = $order_list->where('orders_services_items.order_status',$request->status);  
            }
           ...
           $order_list = $order_list->where('orders_services.created_at','>=',$created)->get();
           $checkwith = 0;
           if($request->status == 11)
           {
             ...
             foreach ($order_list as $key => $value) {
                $check = ServiceOrderRejected::where(['vendor_id'=>$user_id,'service_order_id'=>$value->order_id,'service_order_item_id'=>$value->id])->get()->count();
                if($check == $checkwith)
                {
    ...
    public function service_order_details(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $order = OrderServiceModel::orderBy('order_id','desc')->with(['services' => function ($qr) use ($item_id) {
                ...
                ->join('service', 'service.id', 'orders_services_items.service_id')->where('orders_services_items.id',$item_id);
    ...
    public function service_change_status(Request $request)
    {
        ...
        $check =  ServiceOrderStatusHistory::where('order_id',$order->order_id)->where('status_id',$request->order_status)->where('order_item_id',$request->id)->get()->count();
        if($check == 0)
        {
                $datastatusins = new ServiceOrderStatusHistory;
                $datastatusins->order_id = $order->order_id;
                $datastatusins->order_item_id = $request->id;
