...
function process_order($list, $lang_code = "1")
    ...
    function process_product_data($row, $lang_code = "1")
   {
      ...
      if(!empty($item->image))
      {
         ...
         {
           $product_images[] = (string) get_uploaded_image_url(config('global.upload_path').config('global.product_image_upload_dir').$img);//url(config('global.upload_path').config('global.product_image_upload_dir').$img);
         }
      }
      else
      {
